...
\IfFontExistsTF{Montserrat-Regular.otf}{%
  ...
  \IfFontExistsTF{Montserrat}{\setsansfont{Montserrat}}{%
...
%% --- Márgenes --------------------------------------------------------------
%%  El cuerpo se alinea con el título: PPT x=23.7 mm sobre 338.667 → 11.2 mm.
%%  Margen derecho simétrico, así el cuerpo mide los mismos 137.6 mm que el
%%  título y ningún renglón queda más largo que su encabezado.
\setbeamersize{text margin left=11.2mm, text margin right=11.2mm}
...
%%  título de lámina, cuya caja tiene fondo blanco y taparía lo que hubiera.
%%  --- Franja inferior a sangre -------------------------------------------
%%  Dos filetes al pie, azul encima de verde, de borde a borde.
%%
%%  OJO: esto NO sale de ppt_resultados_IREM_2025_master_logos.pptx. Se buscó
%%  en la lámina, en su layout y en el slide master, y en los otros doce .pptx
%%  del equipo: no está en ninguno. Viene de una versión del master posterior,
%%  vista en una captura, así que las alturas son una lectura de esa imagen y
%%  no una medición del archivo. Si aparece el .pptx que la trae, vuelve a
%%  medirla ahí y corrige estas dos longitudes.
\newlength{\iremFranjaAzul}\setlength{\iremFranjaAzul}{2mm}
\newlength{\iremFranjaVerde}\setlength{\iremFranjaVerde}{2mm}
\newcommand{\iremFranjaNodos}{%
  \fill[iremazul]
    ([shift={(0mm,\iremFranjaVerde)}]current page.south west) rectangle
    ([shift={(0mm,\dimexpr\iremFranjaVerde+\iremFranjaAzul\relax)}]current page.south east);
  \fill[iremverde]
    (current page.south west) rectangle
    ([shift={(0mm,\iremFranjaVerde)}]current page.south east);%
}
%%  Comando público, por si hace falta en una lámina `plain` suelta.
\newcommand{\franjaPie}{%
  \begin{tikzpicture}[remember picture, overlay]\iremFranjaNodos\end{tikzpicture}%
}
...
\newcommand{\logosPie}{%
  \begin{tikzpicture}[remember picture, overlay]\iremLogosNodos\iremFranjaNodos\end{tikzpicture}%
}
...
\setbeamertemplate{title page}{%
  ...
  \sbox{\iremCajaVolanta}{%
    \bfseries\color{grisoscuro}\fontsize{7}{9}\selectfont\hyphenchar\font=-1\relax
    \insertsubtitle}%
...
%%  Los dos \vspace están calibrados midiendo el PNG, no a ojo: con 3.2 mm el
%%  centro óptico del título cae en 8.8 mm, que es el del master, y con 6.85
%%  el primer renglón del cuerpo arranca en 23.95 mm, que es donde el master
%%  abre su marco de texto. Si tocas uno, vuelve a medir los dos.
\setbeamertemplate{frametitle}{%
  \vspace*{3.2mm}%
  \begin{beamercolorbox}[wd=\paperwidth, leftskip=11.2mm, rightskip=11.2mm]{frametitle}%
